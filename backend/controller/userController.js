...
const newUser = async (req, res) => {
  ...
  try {
    ...
    newUser.IdMessage = Math.floor(
      Math.random() * 909898980987 * Math.random()
    );
...
const confirmEmail = async (req, res) => {
  ...
  if (!user) {
    return res.status(300).send("user not find");
  }
  try {
...
const userLogin = async (req, res) => {
  ...
  if (!verifyPassword)
    return res.status(300).send("email or password are not correct");
